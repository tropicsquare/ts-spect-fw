...
This document explains the operations that SPECT exposes to the RISC-V CPU and the way how to interface with SPECT
...
\TropicNote{
    The IN/OUT source configuration is disabled for production firmware.
}
...
\SPECTCommandTable{
    ...
    \SPECTCommandMem{0x0000}{8B}{Result: [0x0B5E55ED, 0xBA11FADE] $\Rightarrow$ verified, else failed}
}
...
\SPECTCommandTable{
    ...
    \SPECTCommandMemHead{Command Buffer (production) / Data RAM In}
    ...
    \SPECTCommandMemHead{Result Buffer (production) / Data RAM Out}
...
\TropicNote{
    Following Ops are implemented, but disabled in production firmware! To enable them for debug
    purposes, macro \texttt{DEBUG_OPS} must be defined in \texttt{main.s} file.
}
